...
from services.serveDataFromUrl import (
    handle_download_data_from_url,
    handle_multiple_image_file_downloads
)

from services.serveUploadedFiles import (
    handle_upload_image_file,
    handle_multiple_image_file_uploads,
    handle_upload_video_file
)
...
app = FastAPI(
    docs_url=None if os.environ.get('docs_url') == 'None' else '/docs',
    redoc_url=None if os.environ.get('redoc_url') == 'None' else '/redoc'
)
...
async def upload_image_files(
    ...
    if fileAmount > int(os.environ.get('MULTIPLE_FILE_UPLOAD_LIMIT')):
        raise HTTPException(
            ...
            detail='Amount of files must not be more than {}'.format(
                os.environ.get('MULTIPLE_FILE_UPLOAD_LIMIT'))
        )
...
async def text_to_generate_qr_image(
        ...
        with_logo: Optional[str] = Query(
            'false',
            description='true/false depending your needs default is {}'.format('false'),
            regex='^(true|false)$'
        ),
        OAuth2AuthorizationCodeBearer=Depends(validate_token)):
