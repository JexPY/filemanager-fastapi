...
headers = {
    'Authorization': 'Bearer {}'.format(random.choice(os.environ.get('FILE_MANAGER_BEARER_TOKEN').split(',')))
}

_ORIGINAL_IMAGE = open('static/pictures/original/dcb8ac79618540688ea36e688a8c3635.png', 'rb')
...
async def test_upload_image_file():
    ...
    image_file = {'file': (
        _ORIGINAL_IMAGE_NAME, _ORIGINAL_IMAGE, 'image/png')}
    ...
    assert Path(os.environ.get('IMAGE_THUMBNAIL_LOCAL_PATH')
                + '/' + imagePaths['thumbnail']).is_file()
    cleaning_service(
